...
Expo Go works for everything currently in the repo. The moment we add custom
native modules (Zetic Melange likely needs one), switch to `bunx expo run:ios`
or an EAS dev client.
...
3. **Rescue tab** — three Fetch.ai agent cards + Agentverse integration.
   Likely a fourth tab, or replace Profile during demo flow.
